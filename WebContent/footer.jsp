<div class="footer bg-dark text-light fot1">
  <div class="container-fluid">
    <div class="row fot2">
      <div class="col-sm fot2">
        <ul>
          <li><a href="index.jsp" class="text-secondary">Homepage</a></li>
          <li><a href="wip.jsp" class="text-secondary">About</a></li>
          <li><a href="https://github.com/lucaam/focus.it/commits/master" class="text-secondary">Changelog</a></li>
          <li><a href="https://github.com/lucaam/focus.it" class="text-secondary">Github project</a></li>
          <li><a href="wip.jsp" class="text-secondary">Shipping information</a></li>
        </ul>
      </div>
      <div class="col-sm fot2">
        <ul>
          <li><a href="cart.jsp" class="text-secondary">Cart</a></li>
          <li><a href="profile.jsp" class="text-secondary">Your profile</a></li>
          <li><a href="https://github.com/lucaam/focus.it/commits/master" class="text-secondary">Changelog</a></li>
          <li><a href="https://github.com/lucaam/focus.it" class="text-secondary">Github project</a></li>
          <li><a href="wip.jsp" class="text-secondary">Shipping information</a></li>
        </ul>
      </div>
      <div class="col-sm fot2">
        <ul>
          <li><a href="searchProduct?search=Canon" class="text-secondary">Canon</a></li>
          <li><a href="searchProduct?search=Leica" class="text-secondary">Leica</a></li>
          <li><a href="searchProduct?search=Nikon" class="text-secondary">Nikon</a></li>
          <li><a href="searchProduct?search=Olympus" class="text-secondary">Olympus</a></li>
          <li><a href="searchProduct?search=Panasonic" class="text-secondary">Panasonic</a></li>
        </ul>
      </div>
    </div>



  </div>
</div>

<div class="footer text-dark fot2">
  <div class="container-fluid">
    <p class="text-center small">
      Copyright &copy; 2018 - Focus.it P.IVA 0000000000000000 - Capitale sociale interamente versato 200.000 - Reg #00000 <br> Funzione di ricerca basata su nulla per adesso - All Rights Reserved - Housing and Web Design by Alessio Cuccurullo, Luca Amoriello

  </div>
</div>
